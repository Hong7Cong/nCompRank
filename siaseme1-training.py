...
ranknet = RankNet_wresnet(feature_extractor=fextract, cotrain = cotrain, simclr = simclr)
ranknet.to(device)
loss_fn = torch.nn.BCELoss()
if(cotrain):
    optimizer = optim.SGD([{'params': ranknet.parameters()}], lr=lr, momentum=momentum)
else:
    if(fextract == 'resnet50' or fextract == 'resnet101'):
...
# optimizer = optim.SGD([{'params': ranknet.parameters()}], lr=lr, momentum=momentum) if cotrain else optim.SGD([{'params': ranknet.fextractor.fc.parameters()}], lr=lr, momentum=momentum)  
...
for e in range(100):
    ...
    if(e % 1 == 0 and issaved != 'nosave'):
        ...
        save_name = f'siaseme1_mode{mode}_fextra{fextract}_lr{lr}_mo{momentum}_e{e}_{todat_str}_l{datalen}_cotrain{cotrain}.pt'
        # torch.save(ranknet.state_dict(), f'./pretrained/mdindex_siamese1/{save_name}')
        # print(f'save locally to ./pretrained/mdindex_siamese1/{save_name}')
        if(issaved == 'dropbox'):
            torch.save(ranknet.state_dict(), f'./pretrained/mdindex_siamese1/{save_name}')
            transferData.upload_file(f'./pretrained/mdindex_siamese1/{save_name}', f'/pretrained-models/{save_name}')
            print(f'save to dropbox ./pretrained-models/{save_name}')
        elif(issaved == 'ggdrive'):
            torch.save(ranknet.state_dict(), f'/content/drive/MyDrive/hongn/pretrained/mdindex_siamese1/{save_name}')
            print(f'save to ggdrive /content/drive/MyDrive/hongn/pretrained/mdindex_siamese1/{save_name}')
        else:
            torch.save(ranknet.state_dict(), f'./pretrained/mdindex_siamese1/{save_name}')
            print(f'save locally to ./pretrained/mdindex_siamese1/{save_name}')
